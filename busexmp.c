...
static int xmp_trim(u_int64_t from, u_int32_t len, void *userdata) {
  if (*(int *)userdata) fprintf(stderr, "T - %lu, %u\n", from, len);
  return 0;
}
...
static void PrintArgument(struct arguments *arguments) {
  printf("SIZE: %llu\n", arguments->size);
  printf("DEVICE: %s\n", arguments->device);
  printf("VERBOSE: %d\n", arguments->verbose);
}
...
int main(int argc, char *argv[]) {
  ...
  int res = buse_main(arguments.device, &aop, (void *)&arguments.verbose);
